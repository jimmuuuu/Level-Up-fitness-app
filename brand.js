(() => {
  ...
  function ensureHeadBranding() {
    if (document.title !== 'Level Up Fitness') document.title = 'Level Up Fitness';
    ...
    if (!favicon.href.endsWith(APP_ICON)) favicon.href = APP_ICON;
    ...
    if (!touchIcon.href.endsWith(APP_ICON)) touchIcon.href = APP_ICON;

    const titleMeta = document.querySelector('meta[name="apple-mobile-web-app-title"]');
    if (titleMeta?.getAttribute('content') !== 'Level Up Fitness') titleMeta?.setAttribute('content', 'Level Up Fitness');
  }
  ...
  function anatomyMap(target) {
    return `<div class="lu-muscle-map" data-target="${target}" aria-hidden="true">
  ...
  function ensureTrainingHeader() {
    ...
    setText(over, 'WORKOUT LIBRARY');
    setText(heading, 'Training');
    ...
    setText(libraryHeading?.querySelector('.over'), 'READY-MADE ROUTINES');
    setText(document.getElementById('libraryTitle'), 'Premade workouts');
    setText(document.getElementById('create'), 'Build custom workout');
  }
  ...
  function renderPremadePlans() {
    ...
    list.innerHTML = plans.map(plan => {
      ...
      const preview = (plan.exercises || []).slice(0, 3).map(exercise => `<div class="lu-premade-exercise"><span>${exercise.name}</span><b>${exercise.muscle || ''}</b></div>`).join('');
      return `<article class="lu-premade-card" data-group="${group}">
        <div class="lu-premade-head"><div class="lu-premade-title"><h3>${displayName}</h3><p>${plan.exercises.length} exercises · ${plan.time}</p></div>${anatomyMap(target)}</div>
        <div class="lu-premade-preview">${preview}</div>
        <div class="lu-premade-footer"><small>${plan.type}</small><button class="lu-premade-view" type="button" data-plan-id="${planId}" data-display-name="${displayName}">View all</button></div>
      </article>`;
    }).join('');
    ...
    list.querySelectorAll('.lu-premade-view').forEach(button => {
      button.onclick = () => {
        ...
        requestAnimationFrame(() => {
          ...
          if (title && button.dataset.displayName) setText(title, button.dataset.displayName);
        });
      };
    });
    list.querySelectorAll('.lu-premade-card').forEach(card => {
  ...
  const observer = new MutationObserver(() => {
    const needsAuthBrand = Boolean(document.querySelector('#authGate .auth-panel')) && !document.querySelector('.level-up-auth-brand');
    const needsWorkoutBrand = Boolean(document.querySelector('#workout > .over')) && !document.querySelector('.level-up-workout-brand');
    const profilePanel = document.getElementById('profileSettingsPanel');
    const needsProfileBrand = Boolean(profilePanel) && !profilePanel.querySelector('.level-up-profile-brand');
    const planList = document.getElementById('planList');
    const needsTraining = Boolean(planList) && !planList.querySelector(':scope > .lu-premade-card');
    if (!needsAuthBrand && !needsWorkoutBrand && !needsProfileBrand && !needsTraining) return;
    if (scheduled) return;
    scheduled = true;
    requestAnimationFrame(() => {
      scheduled = false;
