(() => {
  ...
  async function saveCloud(note, attempt = 0) {
    if (!sessionId()) return;
    try {
      ...
      if ((!rows || !rows.length) && attempt < 4) setTimeout(() => void saveCloud(note, attempt + 1), 1000 * (attempt + 1));
    } catch {
      if (attempt < 3) setTimeout(() => void saveCloud(note, attempt + 1), 1500 * (attempt + 1));
    }
  }

  function ensureCard() {
    const finishSection = document.getElementById('endWorkoutSection');
    if (!finishSection || !sessionId()) return null;
  ...
  function start() {
    ...
    document.addEventListener('click', event => {
      ...
      const note = read();
      if (note) void saveCloud(note);
    }, true);
    setInterval(() => { removeIfInactive(); ensureCard(); }, 1800);
  }

  window.LevelUpWorkoutNotes = { get: read, save: value => { const note = write(value); void saveCloud(note); return note; } };
